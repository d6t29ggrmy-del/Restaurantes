# Restaurant Monitor Robot | v0.5 | 14/08/2026 | descoberta via Google Places API (New) + seed silencioso na 1a rodada
"""
    ...
    for cidade in CIDADES:
        ...
        for p in grupo:
            if mostrados >= MAX_ITENS:
                break
            alvo = p.get("google") or p.get("site") or link_yelp(p["nome"], cidade)
            linhas.append(f"• {p['nome']}{linha_nota(p)}: {alvo}")
            mostrados += 1
    restantes = len(novos) - mostrados
    if restantes > 0:
        linhas.append(f"\n…e mais {restantes} (ver digest).")
    return "\n".join(linhas)
